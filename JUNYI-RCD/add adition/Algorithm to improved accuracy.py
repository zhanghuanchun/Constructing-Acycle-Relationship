...
if __name__ == '__main__':
    # 创建有向图
    df = pd.read_csv('../data/ASSIST/graph/K_Directed_raw.txt', sep='\t',header=None, skiprows=1)
    G = nx.DiGraph()
    ...
    # csv_data = info.to_csv("../data/ASSIST/graph/K_Directed_accessibility.txt",sep='\t',index=None)
    ...
    print(count)
    info = pd.DataFrame(data)
    csv_data = info.to_csv("../data/ASSIST/graph/K_Directed_accessibility1.txt",sep='\t',index=None)
