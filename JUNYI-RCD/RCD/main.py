    ...
    for epoch in range(args.epoch_n):
        ...
        while not data_loader.is_end():
            ...
            if batch_count % 200 == 199:
                with open('result/train_model_val.txt', 'a', encoding='utf8') as f:
...
def predict(args, net, epoch):
    ...
    while not data_loader.is_end():
        ...
        for i in range(len(labels)):
            if (labels[i] == 1 and output[i] > 0.5) or (
                labels[i] == 0 and output[i] < 0.5
            ):
                correct_count += 1
    ...
    print(
        "epoch= %d, accuracy= %f, rmse= %f, auc= %f" % (epoch + 1, accuracy, rmse, auc)
    )
...
if __name__ == "__main__":
    ...
    # net.load_state_dict(torch.load("./model_epoch20.model"))
    # predict(args, net, 20)
